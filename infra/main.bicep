...
@description('Use ACR admin credentials and a storage connection string instead of managed identity/RBAC. Only for subscriptions where Microsoft.Authorization roleAssignments are blocked.')
param useCredentialFallback bool = false
...
// Extensions have to be allow-listed on the server before the migrations can create them
// in the database.
resource allowExtensions 'Microsoft.DBforPostgreSQL/flexibleServers/configurations@2024-08-01' = {
  parent: postgres
  name: 'azure.extensions'
  properties: {
    value: 'VECTOR,PG_TRGM,PGCRYPTO,CITEXT'
...
resource api 'Microsoft.App/containerApps@2024-03-01' = {
  ...
  properties: {
    ...
    configuration: {
      ...
      // Default: pull with the app's own identity. Fallback mode is for subscriptions
      // where Microsoft.Authorization/roleAssignments are blocked, so the Container App
      // cannot be granted AcrPull before its first image pull.
      registries: useCredentialFallback ? [
        {
      ...
      secrets: concat([
        ...
        { name: 'storage-connection-string', value: 'DefaultEndpointsProtocol=https;AccountName=${storage.name};AccountKey=${storage.listKeys().keys[0].value};EndpointSuffix=${az.environment().suffixes.storage}' }
      ] : [])
    }
    template: {
      containers: [
        {
          ...
          env: concat([
            ...
            { name: 'SPRING_MAIL_PASSWORD', value: '' }
          ], useCredentialFallback ? [
            { name: 'ROVER_STORAGE_CONNECTION_STRING', secretRef: 'storage-connection-string' }
          ] : [])
          probes: [
            {
              type: 'Liveness'
...
resource apiBlobAccess 'Microsoft.Authorization/roleAssignments@2022-04-01' = if (!useCredentialFallback) {
  scope: storage
...
resource apiPull 'Microsoft.Authorization/roleAssignments@2022-04-01' = if (!useCredentialFallback) {
  scope: registry
...
resource mlPull 'Microsoft.Authorization/roleAssignments@2022-04-01' = if (!useCredentialFallback) {
  scope: registry
...
resource webPull 'Microsoft.Authorization/roleAssignments@2022-04-01' = if (!useCredentialFallback) {
  scope: registry
