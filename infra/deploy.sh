...
AZURE_CREDENTIAL_FALLBACK="${AZURE_CREDENTIAL_FALLBACK:-false}"
...
if [ "$AZURE_CREDENTIAL_FALLBACK" = "true" ]; then
  echo "==> credential fallback enabled: using ACR admin credentials and a storage connection string"
fi
...
REGISTRY=$(az deployment group create \
  ...
  --parameters namePrefix="$NAME_PREFIX" location="$LOCATION" enableAdminUser="$AZURE_CREDENTIAL_FALLBACK" \
...
az deployment group create \
  ...
  --parameters \
      ...
      location="$LOCATION" \
      useCredentialFallback="$AZURE_CREDENTIAL_FALLBACK" \
